...
`simulate-issues` resets the fork named by `SEED_REPOSITORY_FULL_NAME` to a
pinned baseline plus three poisoned backend bugs, files eight issues over three
minutes, and compares Devin's verdicts with the expected ones. Vocabulary is
in `CONTEXT.md`, the destructive reset is ADR 0004. For a first-time walkthrough
against your own fork, see `DEMO.md`.
...
- `gh` logged in with admin on the fork (also used for Git pushes, run
  ...
  ```sh
  gh api repos/<owner>/superset --jq .permissions.admin   # must print true
  ```
...
If the wait times out (45 min, `--timeout`), the table prints `pending` for
unfinished issues.
