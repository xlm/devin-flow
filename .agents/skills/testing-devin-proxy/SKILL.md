...
2. Start a local HTTP stub that records request method, path, query parameters, Authorization header, and body. The ready-made recording stub at `frontend/e2e/harness/stub-devin.ts` covers v3 playbooks, automations and sessions, v3beta1 repositories, and `/_harness/calls`, which returns recorded requests. Reuse it instead of writing a new stub. `pnpm e2e` wires it up automatically. Configure `DEVIN_API_BASE_URL` to its API v3 base, plus a fake `DEVIN_API_TOKEN` and test `DEVIN_ORG_ID`. Confirm the configured base URL is local before starting the backend.
