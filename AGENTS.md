...
- `backend/` - Python package `devin_flow` (src layout). `main.py` is the
  ...
  `DEVIN_ORG_ID`, the latter two required, `POLL_INTERVAL_SECONDS`,
  default 15, 0 disables the poller, and `PLAYBOOKS_DIR`) come from
  `config.py` (pydantic-settings, `.env` aware). `db.py` builds the sync SQLModel
  ...
  `seed(session, *, playbook_id, repository_full_name)`. `uv run seed` creates
  the Seed Flow when the `Issue triage` Playbook is found by title, targeting
  `SEED_REPOSITORY_FULL_NAME` (default `xlm/superset`), and test fixtures pass
  `playbook_id=None` to keep an empty Canvas.
